...
region
    ...
    i = INDENT? x = LDELIM '@' ID RDELIM {if (input.LA(1)!=NEWLINE) indent=$i;} template INDENT? LDELIM '@end' RDELIM
...
ifstat
    @init {Token indent=null;}:
    i = INDENT? LDELIM 'if' '(' c1 = conditional ')' RDELIM {if (input.LA(1)!=NEWLINE) indent=$i;} t1 = template (
...
notConditionalExpr: (ID -> ID) ( p = '.' prop = ID -> ^(PROP[$p,"PROP"] $notConditionalExpr $prop) | p = '.' '(' mapExpr ')' -> ^(PROP_IND[$p,"PROP_IND"]
            $notConditionalExpr mapExpr) )*
;
...
mapExpr:
                memberExpr (
                    ...
                    | -> memberExpr ) ( {if ($x!=null) $x.clear();} col = ':' x += mapTemplateRef // don't keep queueing x; new list for each iteration
                        ({$c==null}? => ',' x += mapTemplateRef )* -> ^(MAP[$col] $mapExpr $x+ ) )*
;
...
memberExpr: (includeExpr -> includeExpr) ( p = '.' ID -> ^(PROP[$p,"PROP"] $memberExpr ID) | p = '.' '(' mapExpr ')' -> ^(PROP_IND[$p,"PROP_IND"] $
                            memberExpr mapExpr) )*
;
...
includeExpr options {
                        k = 2;
                    }                                                        : // prevent full LL(*), which fails, falling back on k=1; need k=2
...
qualifiedId: (ID -> ID | '/' ID -> ^('/' ID) ) ( '/' r = ID -> ^('/' $qualifiedId $r) )*
;
